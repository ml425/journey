    ...
    <div class="moneyWithdraw">  
        <div class="header">
            ...
            <div class="header_bottom">
                <span>到账银行卡</span>
                <span>
                    <van-icon name="gold-coin-o" color="#07c160"/>**银行({{WaiterInfoData.bankCard?WaiterInfoData.bankCard:'6688'}})
                    <div>2小时内到账</div>
                </span>
            </div>
        </div>
        <div class="content">
            <div>到账金额</div>
            <van-cell-group>
                <van-field v-model="value" label="￥" placeholder="请输入提现金额" />
            </van-cell-group>
            <div>
                <span>当前零钱余额为{{AllMoney}}</span>
                <span @click="allRecharge">全部提现</span>
            </div>
            <van-button round block color='#f1f1f1' @click="recharge">提现</van-button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods:{
        ...mapActions('moneyWithdraw',['tomoneyWithdraw']),
        // 全部提现
        allRecharge(){
            this.value = this.AllMoney
        ...
        recharge(){
            ...
            // console.log(obj);
            this.tomoneyWithdraw(obj).then(r=>{
...
<style scoped>
    ...
    .content>div:nth-child(3)>span:first-child{
        font-size:12px;
        color: rgb(204, 204, 204);
        width: 70%;
        display: inline-block;
